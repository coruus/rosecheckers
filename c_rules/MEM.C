...
bool hasAssignToVar(const SgNode* node,
  ...
  const Rose_STL_Container<SgNode *> nodes
    = NodeQuery::querySubTree(const_cast< SgNode*>( node), V_SgVarRefExp);
  ...
  // if (i != nodes.end()) std::cerr << "i: " << (*i)->get_parent()->unparseToString() << std::endl;
    ...
    if (next_assignment != NULL && isSgVarRefExp(next_assignment->get_lhs_operand()) && 
	next_assignment->get_lhs_operand() == i_ref) {
      // we have an assignment to var, but must ensure var does
      // not appear on the RHS
      hasAssignToVar( next_assignment->get_rhs_operand(), NULL, true, var, violation);
      return true;
    }
...
bool MEM30_C( const SgNode *node ) { // Ensure that freed pointers are not reused
  ...
  if (forLoop) {
    ...
    assignmentFound = hasAssignToVar( forLoop->get_loop_body(),
				      ref, true, var, &violation);
    // check loop increment
    if (!assignmentFound && !violation)
      assignmentFound = hasAssignToVar( forLoop->get_increment(),
					NULL, true, var, &violation);
    // check loop test
    if (!assignmentFound && !violation)
      assignmentFound = hasAssignToVar( forLoop->get_test(),
					NULL, true, var, &violation);
    // check loop body up to the free
    if (!assignmentFound && !violation)
      assignmentFound = hasAssignToVar( forLoop->get_loop_body(),
					ref, false, var, &violation);
  }
  ...
  if (!violation && whileLoop) {
    ...
    assignmentFound = hasAssignToVar( whileLoop->get_body(),
				      ref, true, var, &violation);
    // check loop condition
    if (!assignmentFound && !violation)
      assignmentFound = hasAssignToVar( whileLoop->get_condition(),
					NULL, true, var, &violation);
      // check loop body up to the free
    if (!assignmentFound && !violation)
      assignmentFound = hasAssignToVar( whileLoop->get_body(),
  ...
  if (!violation && doWhileLoop) {
    // check body of loop after call to free
    assignmentFound = hasAssignToVar( doWhileLoop->get_body(),
				      ref, true, var, &violation);
    // check loop condition
    if (!assignmentFound && !violation)
      assignmentFound = hasAssignToVar( doWhileLoop->get_condition(),
					NULL, true, var, &violation);
      // check loop body up to the free
    if (!assignmentFound && !violation)
      assignmentFound = hasAssignToVar( doWhileLoop->get_body(),
  ...
  if (!violation) 
    hasAssignToVar( findParentNodeOfType( node, V_SgFunctionDefinition).first,
		    ref, true, var, &violation);
