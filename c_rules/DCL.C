...
bool DCL04_CPP( const SgNode *node ) { // Declare no more than one variable per declaration
...
bool DCL30_CPP( const SgNode *node ) { // Do not use names reserved for the implementation
	...
	if( result ) {
	  std::string msg = "Do not ignore values returned by functions: " + id;
	  print_error(node, "DCL30-C", msg.c_str(), true);
	}

	return result;
}

bool DCL32_CPP( const SgNode *node ) { // Avoid runtime static initialization of objects with external linkage
	if( const SgVariableDefinition *vdef = isSgVariableDefinition( node ) ) {
		...
		if( sm.isExtern() || sm.isUnknown() || sm.isUnspecified() ) { //XXX not so sure this is right...
			...
			if( runtimeInit ) {
			  print_error(node, "DCL32-C", "Runtime static initialization of object with external linkage", false);
				return true;
			}
			else {
...
bool DCL_CPP(const SgNode *node) {
  bool violation = false;
  violation |= DCL04_CPP(node);
  violation |= DCL30_CPP(node);
  violation |= DCL32_CPP(node);
  return violation;
}
